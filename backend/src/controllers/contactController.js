...
export const submitContactMessage = async (req, res) => {
  try {
    ...
    // 1. Primary Operation: Persist the message in MongoDB first
    ...
    // 2. Secondary Operation: Email notification via Resend
    // Wrapped in an isolated try/catch so email transport errors don't fail the submission
    ...
    try {
      ...
      console.warn("Resend notification failed/skipped:", emailError.message);
    }

    // Always respond 201/200 once the message is stored
    return res.status(201).json({
      success: true,
      message: "Query submitted and recorded successfully.",
    ...
    return res.status(500).json({
      success: false,
      message: "Failed to store contact query.",
    });
  }
};